...
from transformers.utils import add_start_docstrings, add_start_docstrings_to_model_forward, logging, ModelOutput
...
from model.ringattention import blockwise_ffn, ring_flash_attention_tpu, \
...
class FlaxLLaMAAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.causal_mask = make_causal_mask(jnp.ones((1, config.max_sequence_length * config.max_thinking_steps), dtype="bool"), dtype="bool")

        self.freqs_cis = precompute_freqs_cis(
            self.head_dim,
            config.max_sequence_length * config.max_thinking_steps,
            theta=config.theta,
            dtype=self.dtype,
        )
...
class FlaxLLaMAForCausalLMModule(nn.Module):
    ...
    def __call__(
        ...
        return FlaxCausalLMOutputNew(logits=lm_logits, hidden_states=outputs.hidden_states, attentions=outputs.attentions)


@flax.struct.dataclass
class FlaxCausalLMOutputNew(ModelOutput):
    logits: jnp.ndarray = None
    hidden_states: Optional[Tuple[jnp.ndarray]] = None
    attentions: Optional[Tuple[jnp.ndarray]] = None
    cls_logits: jnp.ndarray = None
