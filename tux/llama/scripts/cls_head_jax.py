...
def main(argv):
    ...
    def update(params):
        ...
        initializer = jax.nn.initializers.normal(stddev=llama_config.initializer_range)
        key = jax.random.PRNGKey(42)
        params['cls_head'] = {'kernel': initializer(key, shape=(llama_config.hidden_size, 2)).astype(np.float32)}
